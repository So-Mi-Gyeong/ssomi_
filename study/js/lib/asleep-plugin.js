	...
	"use strict";
		...
		$.extend( Plugin.prototype, {
			...
			init: function() {
				...
				var _this = this;
				var ringbgrd, holder, indicatorArc, handles, dragBehavior, newbg;
...
//https://amdevblog.wordpress.com/2016/07/20/update-d3-js-scripts-from-v3-to-v4/
				...
				$('#ingSleep_check').on('change', function(){
					alert(11)
				})
				...
				var drawHandles = function () {
					...
					var circles = handlerContainer.enter()
						...
						circles.append("image")
							.attr('href', function(d, i) { if(d.label == "a") return '../../../local/assets/img/asleep/ico_sleep_night.svg'; else return '../../../local/assets/img/asleep/ico_sleep_morning.svg' })
							.attr('width','30')
							.attr('height','30')
							.attr('transform','translate(-15,-15)')
							.attr('transform', function(d){
								return 'rotate(' + '-' + angularScale(d.value) + ') translate(-15, -15)'; //initial position
							})	
				...
				var updateHandles = function (handle){
					...
					$('#handles .handlercontainer.active #a + image').attr('transform', function(d,i){

						return 'rotate('+ '-' + angularScale(up) +') translate(-15, -15)';
					});
					$('#handles .handlercontainer.active #e + image').attr('transform', function(d,i){
				...
				newbg = this.graph.svg
					.append('g')
					.attr('id','newbg').attr('transform','translate('+radius+','+radius+')');
				...
				newbg.append('circle')
					...
					.attr('stroke', this.settings.newbg)
			...
			return this.each( function() {
				if ( !$.data( this, "plugin_" + pluginName ) ) {
					...
					setInterval(function(){
						options.data.end = OcareUI.state.time.value();
						console.log('options', options.data.end);
